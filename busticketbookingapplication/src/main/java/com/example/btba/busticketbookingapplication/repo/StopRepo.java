...
public interface StopRepo extends JpaRepository<Stop, Long> {
    ...
    @Query("select distinct s1.route.id from Stop s1 join Stop s2 on s1.route.id = s2.route.id where s1.location like concat('%', :source, '%') AND s2.location like concat('%', :destination, '%')")
